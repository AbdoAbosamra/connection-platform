<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-500">Browse remote opportunities from top companies worldwide</p>
    </div>
        ...
        <div class="card p-5 space-y-5 sticky top-20">
          ...
          <div>
            ...
            <select v-model="filters.experience_level" class="input !py-2.5 bg-gray-50 cursor-pointer">
              ...
              <option value="entry">Entry level{{ fc('experience_level', 'entry') }}</option>
              <option value="mid">Mid level{{ fc('experience_level', 'mid') }}</option>
              <option value="senior">Senior level{{ fc('experience_level', 'senior') }}</option>
              <option value="lead">Lead{{ fc('experience_level', 'lead') }}</option>
              <option value="executive">Executive{{ fc('experience_level', 'executive') }}</option>
            </select>
          </div>
          ...
          <div>
            ...
            <select v-model="filters.employment_type" class="input !py-2.5 bg-gray-50 cursor-pointer">
              <option value="">Any</option>
              <option value="full_time">Full-time{{ fc('employment_type', 'full_time') }}</option>
              <option value="part_time">Part-time{{ fc('employment_type', 'part_time') }}</option>
              <option value="contract">Contract{{ fc('employment_type', 'contract') }}</option>
              <option value="freelance">Freelance{{ fc('employment_type', 'freelance') }}</option>
              <option value="internship">Internship{{ fc('employment_type', 'internship') }}</option>
            </select>
          </div>
...
// Facet count suffix for a filter option, e.g. " (12)". Empty when there are
// no matches so the option label stays clean.
function fc(dimension, value) {
  const n = store.facets?.[dimension]?.[value]
  return n ? ` (${n})` : ''
}
...
function resetFilters() {
  Object.assign(filters, {
    q: '', category: '', experience_level: '', employment_type: '', page: 1,
  })
  doSearch()
}
