...
onMounted(async () => {
  const fallback = { data: {} }
  const [profileRes, recRes, appsRes, statsRes] = await Promise.all([
    client.get('/job-seeker/profile').catch(() => fallback),
    jobsApi.recommended().catch(() => ({ data: { jobs: [] } })),
    applicationsApi.myApplications({ per_page: 5 }).catch(() => ({ data: { data: [], total: 0 } })),
    client.get('/job-seeker/stats').catch(() => fallback),
  ])
  profile.value         = profileRes.data.profile ?? null
  recommended.value     = recRes.data.jobs ?? []
  recentApps.value      = appsRes.data.data ?? []
  const s               = statsRes.data
  stats.value[0].value  = s.total_applications   ?? appsRes.data.total ?? 0
  stats.value[1].value  = s.saved_jobs           ?? 0
  stats.value[2].value  = s.interviews_scheduled ?? 0
  stats.value[3].value  = s.offers               ?? 0
